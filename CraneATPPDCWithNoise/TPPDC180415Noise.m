...
for i=1:IterationTimes
    ...
    X(:,i)=x1_0;
     % model of cranes
    tpdc_u(i) = towercraneux_CalUq(x1_0,K1,U1,domain,n1,q);
    [t,x]=ode45(@SPG_plantnoise,[0,Tspan],x1_0,opt,tpdc_u(i),lpvux,dt);
    q=q+(y_r-x1_0(1))*Tspan;
    X_PDC(:,i) = x(end,:)';
    x1_0=x(end,:)';
end
ParaStruct.time=time;
ParaStruct.X_PDC=X_PDC;
ParaStruct.tpdc_u=tpdc_u;
Hnamemat=strcat('TPPDC180415Noise','.mat');
save(Hnamemat,'time','X_PDC','tpdc_u')
