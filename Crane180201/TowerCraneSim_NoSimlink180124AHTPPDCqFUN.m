function ParaStruct=AHTTPPDCMain()
%% HTPPDC ¸ú×Ù¿ØÖÆÆ÷
% 2018-01-26
clc
clear
...
% epsilon = 0.2;
    ...
    domain = [-5/180*pi 5/180*pi; -0.45 0.45];
    ...
    [S1,U1,maxerr,meanerr]=TPtransformaiton(TP,hull,svtol, keep,primebase,pbaseindx);
    phi = 1;
    ind=0;
    ...
    save('ParaStructDataH.mat','ParaStruct')
    load('ParaStructDataAH.mat', 'ParaStruct')
    datapfea=reshape(cell2mat(ParaStruct.feasible),[],length(data))
    figure(1)
...
%%%%%%%%%%%%%%%%%%%%%%
function [S1,U1,maxerr,meanerr]=TPtransformaiton(TP,hull,svtol, keep,primebase,pbaseindx)
%% TP transformation, same as:
    %   [S U] = tptrans(lpv, dep1, domain, gridsize, 'close');
    % sampling
    p = primes(primebase);    
    p=p(pbaseindx:end); %  p_1=5 5x2
    Xij= DIST_HAMMERSLEYDiy(TP.domain,TP.siz,p);
    TP.X_scaled=Xij;
    lpvdata1 = sampling_lpvada(TP);
    %lpvdata1 = sampling_lpv(lpvux, dep1, domain1, gridsize1);
    % hosvd
    % [S U sv tol] = hosvd_lpv(data, dep, gridsize, svtol, keep)

    [S1 U1 sv tol] = hosvd_lpv(lpvdata1, TP.dep, TP.gridsize, svtol, keep);

    % generating tight polytopic representation
    U1 = genhull(U1, hull);
    S1 = coretensor(U1, lpvdata1, TP.dep);

    % check model approximation error
    [maxerr meanerr] = tperror(TP.lpv, S1, U1, TP.domain, 100);
